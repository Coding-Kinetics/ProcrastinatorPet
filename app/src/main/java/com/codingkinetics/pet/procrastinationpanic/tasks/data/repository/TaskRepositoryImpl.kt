    ...
    constructor(
        ...
        override suspend fun getTask(id: Int): Result<Task> {
            return try {
                ...
                logger.logError(tag, "Unable to get task $id. Cause:" + e.message.toString())
                Result.Failure(e)
            }
        }
        ...
        override suspend fun getAllTasks(): Result<List<Task>> {
            return try {
                ...
                logger.logError(tag, "Unable to retrieve all tasks. Cause:" + e.message.toString())
                Result.Failure(e)
            }
        }

        override suspend fun createTask(task: Task): Result<List<Task>> {
            return try {
                when (val task = localDataSource.insertTask(task.toTask(tag, logger))) {
                    is Result.Success -> {
                        val taskDomain = task.data.toTaskDomain(tag, logger)
                        Result.Success(taskDomain)
                    }
                ...
                logger.logError(tag, "Unable to create task ${task.title}. Cause:" + e.message.toString())
                Result.Failure(e)
            }
        }
